...
bool PseudoConstantAnalysis::isPseudoConstant(const ValueDecl *VD, const Stmt *Allowed) {
  ...
  std::map<const ValueDecl*, std::vector<const Stmt*>>::iterator it = Edits.find(VD);
  return it == Edits.end() || ( it->second.size()==1 && it->second[0] == Allowed );
}

// Returns true if the variable was used (self assignments don't count)
bool PseudoConstantAnalysis::wasReferenced(const ValueDecl *VD) {
  if (!Analyzed) {
    RunAnalysis();
    Analyzed = true;
  }
...
void PseudoConstantAnalysis::RunAnalysis() {
  ...
  while (!WorkList.empty()) {
    ...
    case Stmt::BinaryOperatorClass: {
      ...
      case BO_ShlAssign:
      case BO_ShrAssign: {
        const ValueDecl *VD = dyn_cast<VarDecl>(LHSDecl);
        if (!VD) VD = dyn_cast<FieldDecl>(LHSDecl);
        // The DeclRefExpr is being assigned to - mark it as non-constant
        if (VD) {
          NonConstants->insert(VD);
          Edits[VD].push_back(BO);
        }
        break;
      }
    ...
    case Stmt::UnaryOperatorClass: {
      ...
      case UO_AddrOf: {
        ...
        // non-constant.
        const ValueDecl *VD = dyn_cast<VarDecl>(D);
        if (!VD) VD = dyn_cast<FieldDecl>(D);
        if (VD) {
          NonConstants->insert(VD);
          Edits[VD].push_back(UO);
        }
        break;
      }
    ...
    case Stmt::DeclStmtClass: {
      ...
      for (DeclStmt::const_decl_iterator I = DS->decl_begin(),
        ...
        if (const FieldDecl *RefVD = dyn_cast<FieldDecl>(D)) {
          NonConstants->insert(RefVD);
          continue;
        }
      }
    ...
    case Stmt::DeclRefExprClass: {
      ...
      if (const FieldDecl *VD = dyn_cast<FieldDecl>(DR->getDecl())) {
        // Add the Decl to the used list
        UsedVars->insert(VD);
        continue;
      }
