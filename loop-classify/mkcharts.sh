...
if [[ $2 == "run" ]] ; then
    ...
    ninja || exit 1
    bin/loop-classify $FILES -loop-stats -debug -- -w $INCLUDES 2>&1 >$BENCH.stats | egrep -v '^Args:'
fi
...
WHILE=$(cat "${BENCH}.stats" | egrep '^WHILE-' | awk 'function stripl(s) { return substr(s, 0, length(s)-1) } { printf "%s/%s,", stripl($4), $1; SUM=SUM+(stripl($4)) } END { printf "%.1f/REST\n", 100-SUM; }' | sed 's/,$//' | sed 's/_/\\_/g')
...
\section{WHILE Loops}
\begin{tikzpicture}
\pie[text=pin,radius=6,rotate=270]{$WHILE}
\end{tikzpicture}
\end{document}
EOF
